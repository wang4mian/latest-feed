...
function splitCommandLine(commandLine) {
  ...
  while ((0, _indexOf.default)(commandLine).call(commandLine, doubleDoubleQuote) > -1) doubleDoubleQuote += '@'; //  Replace all pairs of double-quotes with above marker.


  var noDoubleDoubleQuotes = commandLine.replace(/""/g, doubleDoubleQuote); //  As above, find a unique marker for spaces.

  var spaceMarker = '<SP>';

  while ((0, _indexOf.default)(commandLine).call(commandLine, spaceMarker) > -1) spaceMarker += '@'; //  Protect double-quoted strings.
  ...
  var mangledParamArray = noSpacesInQuotes.split(/ +/); //  Create a new array by restoring spaces from any space-markers. Also, any
  //  remaining double-double-quote markers must have been from OUTSIDE a double-
  //  quoted string and so are removed.

  var paramArray = (0, _map.default)(mangledParamArray).call(mangledParamArray, mangledParam => {
