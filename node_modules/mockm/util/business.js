  ...
  */
  ...
  function initHandle() {
    ...
    function configFileFn({
      ...
      if (cliArg[`--config`] === true && hasCwdConfig === false) {
        // 如果 config=true 并且当前目录没有配置时, 则生成示例配置并使用
        tool.file.createFile({
          filePath: cwdConfigPath,
          str: example
        }) && print(tool.cli.colors.yellow(`已从示例配置自动创建 => ${cwdConfigPath}`));
        res = cwdConfigPath;
      } else if (cliArg[`--config`] === true && hasCwdConfig === true) {
        // 使用生成的示例配置
        res = cwdConfigPath;
      } else if (typeof cliArg[`--config`] === `string`) {
        // 命令行上指定的 config 文件
        res = cliArg[`--config`];
        tool.file.createFile({
          filePath: res,
          str: example
        }) && print(tool.cli.colors.yellow(`已从示例配置自动创建 => ${res}`));
      } else if (tool.file.hasFile(cwdConfigPath)) {
        // 命令运行位置下的配置
        res = cwdConfigPath;
      }
    ...
    function init() {
      // 初始化, 例如创建所需文件, 以及格式化配置文件
      const fs = require(`fs`);

      const fileStore = tool.file.fileStore;

      if (tool.file.hasFile(global.config.dataDir) === false) {
        // 初始化 dataDir
        fs.mkdirSync(global.config.dataDir, {
          recursive: true
        });
      }
      ...
      fileStore(global.config.apiWeb, {
        // 初始化 apiWeb
        paths: {},
        disable: []
      });
      fileStore(global.config._share, {
      ...
      }).set(`config`, global.config);
      {
        ...
        if ((0, _keys.default)(newDb).length) {
          const o2s = tool.obj.o2s;

          if (tool.file.isFileEmpty(global.config.dbJsonPath) || global.config.dbCover) {
            // 如果 db 文件为空或声明总是覆盖, 都重写整个文件
      ...
      }
      {
        ...
        tool.file.isFileEmpty(global.config._gitIgnore.file) && fs.writeFile(global.config._gitIgnore.file, (0, _trim.default)(_context27 = tool.string.removeLeft(global.config._gitIgnore.content)).call(_context27), () => {});
      }
      {
        var _context28;

        // 初始化错误日志保存文件
        tool.file.isFileEmpty(global.config._errLog) && fs.writeFile(global.config._errLog, (0, _trim.default)(_context28 = tool.string.removeLeft(`
          readme:
      ...
      }
      {
        ...
        const openApiList = Boolean(global.config.openApi && global.config.backOpenApi) === false ? [] : {
          ...
          object: () => (0, _values.default)(global.config.openApi)
        }[tool.type.isType(global.config.openApi)]();

        const backFn = () => {
          (0, _forEach.default)(openApiList).call(openApiList, item => {
            tool.file.backUrl(global.config._openApiHistoryDir, item, data => {
              // 格式化 openApi 后再保存, 避免压缩的内容不易比较变更
              return (0, _stringify.default)(tool.obj.sortObj(JSON.parse(data)), null, 2); // 排序 obj, 因为 openApi 中的顺序不确定会导致变更过多
            });
          });
        };

        backFn();
        (0, _setInterval2.default)(backFn, global.config.backOpenApi * 60 * 1000);
      }
    }
  ...
  function historyHandle() {
    ...
    */
    ...
    function getRawHistory() {
      ...
      let list = [];
      list = (0, _reduce.default)(_context29 = (0, _keys.default)(history)).call(_context29, (acc, cur) => {
        return (0, _concat.default)(acc).call(acc, history[cur]);
      }, []);
      return list;
    }
    /**
    ...
    function getHistoryList({
      ...
      list = (0, _filter.default)(_context30 = (0, _map.default)(_context31 = (0, _filter.default)(list).call(list, item => item.data)).call(_context31, ({
        path,
        fullApi,
        id,
        data: {
        ...
        const {
          method,
          url
        } = tool.url.fullApi2Obj(fullApi);

        if (methodRef && apiRef) {
          if ((method === methodRef && url === apiRef) === false) {
            // 如果没有找到就返回, 找到才进入数据处理
            return false;
          }
        }
    ...
    function getHistory({
      ...
      // 获取指定 fullApi/id 中的历史记录
      const history = global.HTTPHISTORY;

      if (fullApi === undefined && id) {
        var _context32;

        return (0, _find.default)(_context32 = getRawHistory({
          history
        })).call(_context32, item => item.id === id) || {};
      }

      const {
        path
      } = tool.url.fullApi2Obj(fullApi);
      const list = (0, _filter.default)(_context33 = (0, _reverse.default)(_context34 = [...(history[path] || [])]).call(_context34)).call(_context33, item => // 传入 id 时比较 id
      (id === undefined ? true : item.id === id) && item.fullApi === fullApi);
      const res = find ? find(list) : list[0] || {};
      return res;
    }

    function ignoreHttpHistory({
      req
    }) {
      // 是否应该记录 req
      const {
      ...
      function disableRecord() {
        ...
        if (tool.type.isType(disableRecord, `boolean`)) {
          ...
          return (0, _some.default)(_context35 = global.config.disableRecord).call(_context35, item => {
      ...
      return Boolean(method.match(/OPTIONS/i) || method.match(/GET/i) && url.match(new RegExp(`//${global.config._proxyTargetInfo.pathname}//`)) || disableRecord());
    }
    ...
    function createBodyPath({
      ...
      // 根据 url 生成文件路径, reqOrRes: req, res
      ...
      const {
        url,
        path
      } = tool.httpClient.getClientUrlAndPath(req.originalUrl);
      let {
        method
      } = req;
      method = method.toLowerCase();
      ...
      const newPath = () => {
        ...
        let basePath = osPath.relative(process.cwd(), global.config._requestDir); // 获取相对路径下的 dataDir 目录

        basePath = osPath.isAbsolute(basePath) ? basePath : `./${basePath}`;
        const apiDir = osPath.normalize(`${basePath}/${path}`).replace(/\\/g, `/`); // 以 path 创建目录, 生成相对路径以避免移动 dataDir 后无法使用

        if (tool.file.hasFile(apiDir) === false) {
          // 如果不存在此目录则进行创建
          fs.mkdirSync(apiDir, {
            recursive: true
          });
        ...
        let shortUrl = (0, _indexOf.default)(url).call(url, path) === 0 ? url.replace(path, ``) : url; // 为了节约目录长度删除 url 中的 path 部分, 因为 pathDir 已经是 path 的表示

        shortUrl = (0, _slice.default)(shortUrl).call(shortUrl, 1, 100);
        const filePath = `${apiDir}/${filenamify(`${shortUrl}_${method}_${apiId}_${reqOrRes}${isHeader ? `_h` : ``}.${extensionName}`, {
          maxLength: 255,
          replacement: `_`
    ...
    function createHttpHistory({
      ...
      const headersObj = {
        req: req.headers || req.getHeaders(),
        res: res.headers || res.getHeaders()
      };
      ...
      const reqBody = req.body; // 保存 body 数据文件, 由于操作系统对文件名长度有限制, 下面仅取 url 的前 100 个字符, 后面自增
      ...
      function getBodyPath() {
        ...
        return {
          headersPathReq: createBodyPath({ ...arg,
            reqOrRes: `req`,
            isHeader: true
          }),
          headersPathRes: createBodyPath({ ...arg,
            reqOrRes: `res`,
            isHeader: true
          }),
          bodyPathReq: tool.type.isEmpty(reqBody) === false ? createBodyPath({ ...arg,
            reqOrRes: `req`
          }) : undefined,
          bodyPathRes: tool.type.isEmpty(buffer) === false ? createBodyPath({ ...arg,
            reqOrRes: `res`
          }) : undefined
        };
      }
      ...
      fs.writeFileSync(headersPathReq, (0, _stringify.default)(headersObj.req), {
        encoding: `utf8`
      });
      fs.writeFileSync(headersPathRes, (0, _stringify.default)(headersObj.res), {
        encoding: `utf8`
      });
      bodyPathReq && fs.writeFileSync(bodyPathReq, (0, _stringify.default)(reqBody), {
        encoding: `utf8`
      });
      bodyPathRes && fs.writeFileSync(bodyPathRes, buffer, {
        encoding: `buffer`
      });
      const resDataObj = {
        req: {
          lineHeaders: {
            ...
            headPath: headersPathReq // _header: proxyRes.req._header,
        ...
        res: {
          lineHeaders: {
            ...
            headPath: headersPathRes // _header: res._header,
    ...
    function setHttpHistory({
      ...
      history[path] = (0, _concat.default)(_context36 = history[path] || []).call(_context36, data);
      fs.writeFileSync(global.config._httpHistory, tool.obj.o2s(history));
    }

    function setHttpHistoryWrap({
      req,
      res,
      mock = false,
      buffer
    }) {
      // 从 req, res 记录 history
      if (ignoreHttpHistory({
        req
      }) === false) {
        const data = [];
        const arg = {
        ...
        clientInjection().setApiInHeader({
          req,
          res
        });
        ...
        if (mock === true) {
          ...
          let isSave = false; // eslint-disable-next-line no-inner-declarations

          function saveHistory(ev) {
            if (isSave === false) {
              // 没有保存过才进行保存
              const buffer = (0, _concat.default)(Buffer).call(Buffer, data);
              createHttpHistory({ ...arg,
                buffer
              });
              isSave = true;
    ...
    function clearHistory() {
      function getDelIdList(list, options = {}) {
        ...
        list = (0, _filter.default)(list).call(list, item => {
          // 获取多少分钟之前的请求
          return (0, _now.default)() - new Date(item.date).getTime() > options.retentionTime * 60 * 1000;
        });
        let obj = {}; // 合并相同内容为对象

        list = (0, _sort.default)(_context37 = tool.array).call(_context37, list, {
          key: `id`
        });
        (0, _forEach.default)(list).call(list, ({
          id,
          resBodyMd5,
          reqBodyMd5,
          fullApi,
          ...
          if (options.num < 0) {
            ...
            obj[tag] = (0, _reverse.default)(_context38 = obj[tag]).call(_context38);
          }
        });
        let delIdList = []; // 获取要删除的 ID 列表

        (0, _forEach.default)(_context39 = (0, _keys.default)(obj)).call(_context39, key => {
          var _context40;

          delIdList = (0, _concat.default)(delIdList).call(delIdList, (0, _slice.default)(_context40 = obj[key]).call(_context40, Math.abs(options.num)));
        });
        return delIdList;
      }
      ...
      let list = business().historyHandle().getHistoryList({
        md5: true
      });
      const delIdList = {
        ...
        undefined: () => []
      }[tool.type.isType(global.config.clearHistory)](list); // 删除文件

      const fs = require(`fs`);

      (0, _forEach.default)(delIdList).call(delIdList, id => {
        let {
          reqBodyPath,
          resBodyPath,
          path
        } = (0, _find.default)(list).call(list, item => item.id === id) || {};
        const apiList = HTTPHISTORY[path] || [];

        const findIndex = id => (0, _findIndex.default)(apiList).call(apiList, item => item.id === id); // 删除 json 中的记录


        (0, _splice.default)(apiList).call(apiList, findIndex(id), 1);
  ...
  function clientInjection() {
    // 到客户端前的数据注入, 例如 添加测试 api, 统一处理数据格式
    function setHeader(reqOrRes, headerObj = {}) {
      var _context41;

      reqOrRes.setHeader = reqOrRes.setHeader || reqOrRes.set || function (key, val) {
        reqOrRes.headers[key] = val;
      };

      (0, _forEach.default)(_context41 = (0, _keys.default)(headerObj)).call(_context41, key => {
    ...
    function allowCors({
      ...
      if (global.config.cors === false) {
        // config.cors 为 false 时, 则不允许跨域
        return false;
      }
      /**
       * 这可能是服务器需要的信息
      ...
      if (req) {
        setHeader(req, {
          ...
          'origin': target,
          // 不应包含任何路径信息
          'host': new _url.default(target).host
        });
      }
      /**
       * 这是浏览器需要的跨域信息
      ...
      if (res && res.headersSent !== true) {
        ...
        const rawHeadersObj = (0, _reduce.default)(_context42 = req.rawHeaders).call(_context42, (acc, cur, index) => {
          /**
        ...
        setHeader(res, {
          ...
          // 当为 true 时 origin 不能为 *, 不过我们并没有优先使用 *
          'access-control-allow-origin': rawHeadersObj[`origin`] || `*` // 'access-control-max-age': undefined,
    ...
    function reSetApiInHeader({
      ...
      const testUrlRemote = global.config.remote ? (headers[apiInHeader + `-remote`] || ``).replace(/(.+?)(\/#\/.*)/, `${note.remote.testPort}$2`) : undefined;
      const obj = tool.obj.removeEmpty({
        [apiInHeader]: testUrl,
        [apiInHeader + `-remote`]: testUrlRemote
      });
      obj[`Access-Control-Expose-Headers`] = (0, _keys.default)(obj).join(`,`);
      return obj;
    }

    function setApiInHeader({
      req,
      res
    }) {
      // 设置 testApi 页面到 headers 中
      const store = tool.file.fileStore(global.config._store);
      const note = store.get(`note`);
  ...
  function reqHandle() {
    ...
    function sendReq({
      ...
      // 发送请求
      const fs = require(`fs`); // api httpHistory 中的 api
      ...
      http({
        ...
        method,
        url: pathOrUrl,
        // 注意不要 url 和 params 上都同时存在 query
        params: query,
        headers,
        data: method.toLowerCase() === `get` ? undefined : httpDataReq.bodyPath ? fs.readFileSync(httpDataReq.bodyPath) : {},
    ...
    function injectionReq(arg) {
      ...
      if (type === `get`) {
        new _promise.default(() => {
          ...
          (0, _forEach.default)(_context43 = (0, _entries.default)(global.config.updateToken)).call(_context43, ([formKey, toKey]) => {
            const fn = {
              string: () => {
      ...
      if (type === `set`) {
        ...
        (0, _forEach.default)(_context44 = (0, _entries.default)(global.STORE.get(`updateToken`))).call(_context44, ([key, value]) => {
  ...
  function plugin() {
    ...
    async function runNgrok({
      ...
      await tool.generate.initPackge(`ngrok`, {
        getRequire: false
      });
      ...
      const fs = require(`fs`); // ngrok 要求强制要求未注册用户使用最新版本


      await spawn(`npx`, `ngrok update`.split(/\s+/), {
        // stdio: [0, `pipe`, 2],
        cwd: mainPath
      }); // 获取未占用的 tcp 端口, 用于 ngrok 的 web_addr, 会生成一个 api 供我们调用

      const portList = await _promise.default.all((0, _map.default)(_context45 = [4040, 4041, 4042]).call(_context45, item => getPort(item))).catch(err => console.log(`err`, err)); // 使用这些端口以及用户配置生成 ngrok yaml 格式的配置文件

      (0, _forEach.default)(portList).call(portList, (freePort, index) => {
        const {
          name,
          config
        } = serverList[index];
        ...
        const yamlStr = yaml.stringify(json); // todo 注意 configPath 路径中暂不支持空格
      ...
      const urlList = [];
      await _promise.default.all((0, _map.default)(portList).call(portList, (item, index) => {
        return tool.control.awaitTrue({
          timeout: 30e3,
          condition: () => {
            // 等待 /api/tunnels 接口返回所需的 url
            return new _promise.default(async resolve => {
    ...
    async function remoteServer({
      ...
      const serverList = (0, _map.default)(_context46 = [`port`, `replayPort`, `testPort`]).call(_context46, name => ({
        name,
        config: {
          proto: `http`,
      ...
      const urlList = await runNgrok({
        serverList,
        shareConfig
      }).catch(err => console.log(`err`, err));
      print(`The remote service is loaded.`);
      (0, _forEach.default)(serverList).call(serverList, (item, index) => {
        store.set(`note.remote.${item.name}`, urlList[index]);
        const msg = [`Interface forwarding: ${store.get(`note.remote.${item.name}`) || ``} => http://${shareConfig.osIp}:${shareConfig.port}/`, ``, `Interface list:       ${store.get(`note.remote.${item.name}`) || ``}/#/apiStudio/ => http://${shareConfig.osIp}:${shareConfig.testPort}/#/apiStudio/`][index];
  ...
  function getProxyConfig(userConfig = {}) {
    ...
    const defaultConfig = {
      ...
      logLevel: `silent` // proxyTimeout: 60 * 1000,
      // timeout: 60 * 1000,

    }; // 为了默认注入一些功能, 例如历史记录功能, 需要把用户添加的函数与程序中的函数合并

    (0, _forEach.default)(_context47 = (0, _keys.default)(defaultConfig)).call(_context47, key => {
  ...
  function build() {
    function getBuildStr(packageJson) {
      ...
      if (buildInfo && buildInfo.hash) {
        ...
        return `${packageJson.version} => ${buildInfo.branch}/${(0, _slice.default)(_context48 = buildInfo.hash).call(_context48, 0, 7)}`;
      } else {
