...
import { R as RSS_SOURCES } from '../../chunks/rss-sources_BBGHTes5.mjs';
export { renderers } from '../../renderers.mjs';

const POST = async () => {
  try {
    console.log("[RSS-Init] Starting to initialize RSS sources...");
    const { data: insertedSources, error } = await supabase.from("rss_sources").upsert(RSS_SOURCES, {
      onConflict: "id",
...
const GET = async () => {
  try {
    const { data: dbSources, error } = await supabase.from("rss_sources").select("*").order("created_at", { ascending: false });
    if (error) {
